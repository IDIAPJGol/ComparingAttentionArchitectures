...
class DomainAttentionModel (tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        vars_out, varsAttention = self.varsNN_Att(tf.transpose(inputs_merged, perm = [0, 2, 1]))
        ...
        print("Input logits:", tf.concat([vars_out, years_out, static], axis = -1).shape)
        logits = self.fc0(tf.concat([vars_out, years_out, static], axis=-1))
